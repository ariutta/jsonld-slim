(function (global, factory) {
  ...
  const _isArray = function(v) {
    return Array.isArray(v);
  }

  const _isObject = function(v) {
    return (Object.prototype.toString.call(v) === '[object Object]');
  }

  const _clone = function(value) {
  ...
  const NormalizeHashDOThashNQuads = function(algorithm, nquads) {
  ...
  const Permutator = function(list) {
    // original array
    this.list = list.sort();
  ...
  const IdentifierIssuer$1 = function(prefix) {
    this.prefix = prefix;
    this.counter = 0;
    this.existing = {};
  }

  IdentifierIssuer$1.prototype.clone = function() {
    var copy = new IdentifierIssuer$1(this.prefix);
  ...
  IdentifierIssuer$1.prototype.getId = function(old) {
  ...
  IdentifierIssuer$1.prototype.getName = IdentifierIssuer$1.prototype.getName;


  IdentifierIssuer$1.prototype.hasId = function(old) {
    return (old in this.existing);
  };


  IdentifierIssuer$1.prototype.isNamed = IdentifierIssuer$1.prototype.hasId;
  ...
  const _toNQuad = function(triple, graphName) {
  ...
  const _isUndefined = function(v) {
    return (typeof v === 'undefined');
  }

  const _compareRDFTriples = function(t1, t2) {
  ...
  const JsonLdError = function(msg, type, details) {
  ...
  const _parseNQuads = function(input) {
  ...
  var Normalize = function(options) {
    ...
    this.canonicalIssuer = new IdentifierIssuer$1('_:c14n');
    this.quads = [];
    this.schedule = {};
          ...
          self.waterfall([
            function(callback) {
              self.forEach(idList, function(id, idx, callback) {
                ...
                var issuer = new IdentifierIssuer$1('_:b');
  ...
  const NormalizeHash = function(algorithm) {
  ...
  const _compareShortestLeast = function(a, b) {
  ...
  const _isString = function(v) {
    return (typeof v === 'string' ||
      Object.prototype.toString.call(v) === '[object String]');
  }

  const _isKeyword = function(v) {
    if(!_isString(v)) {
      return false;
    }
    switch(v) {
  ...
  const _removeBase = function(base, iri) {
    // skip IRI processing
    if(base === null) {
      return iri;
    }
  ...
  const _isSubjectReference = function(v) {
  ...
  const _compactIri = function(activeCtx, iri, value, relativeTo, reverse) {
    // can't compact null
    if(iri === null) {
      return iri;
    }
  ...
  const _isAbsoluteIri = function(v) {
    return _isString(v) && v.indexOf(':') !== -1;
  }

  const _isEmptyObject = function(v) {
    return _isObject(v) && Object.keys(v).length === 0;
  }

  const _validateTypeValue = function(v) {
  ...
  const _createTermDefinition = function(activeCtx, localCtx, term, defined) {
    if(term in defined) {
  ...
  const _prependBase = function(base, iri) {
    ...
    // parse given IRI
    var rel = jsonldDOTurl.parse(iri);
  ...
  const jsonldDOTprependBase = function(base, iri) {
    return _prependBase(base, iri);
  };

  const _expandIri = function(activeCtx, value, relativeTo, localCtx, defined) {
    // already expanded
  ...
  const jsonldDOTgetContextValue = function(ctx, key, type) {
    var rval = null;
  ...
  const _compactValue = function(activeCtx, activeProperty, value) {
    // value is a @value
    if(_isValue(value)) {
      // get context rules
  ...
  const jsonldDOTcompareValues = function(v1, v2) {
    // 1. equal primitives
    if(v1 === v2) {
      return true;
    }
  ...
  const jsonldDOThasProperty = function(subject, property) {
    var rval = false;
  ...
  const jsonldDOThasValue = function(subject, property, value) {
    var rval = false;
  ...
  const jsonldDOTaddValue = function(subject, property, value, options) {
    options = options || {};
  ...
  const _addFrameOutput = function(parent, property, output) {
    if(_isObject(parent)) {
  ...
  const jsonldDOTremoveProperty = function(subject, property) {
    delete subject[property];
  };

  const jsonldDOTgetValues = function(subject, property) {
  ...
  const jsonldDOTremoveValue = function(subject, property, value, options) {
    options = options || {};
    if(!('propertyIsArray' in options)) {
      options.propertyIsArray = false;
    }
  ...
  const _removeEmbed = function(state, id) {
    // get existing embed
  ...
  const _filterSubject = function(subject, frame, flags) {
    ...
    if('@type' in frame &&
      ...
      for(var i = 0; i < types.length; ++i) {
        ...
        if(jsonldDOThasValue(subject, '@type', types[i])) {
          return true;
        }
      }
      return false;
  ...
  const _filterSubjects = function(state, subjects, frame, flags) {
  ...
  const _validateFrame = function(frame) {
  ...
  const _getFrameFlag = function(frame, options, name) {
    var flag = '@' + name;
  ...
  const _createsCircularReference = function(subjectToEmbed, subjectStack) {
  ...
  const _createImplicitFrame = function(flags) {
    var frame = {};
  ...
  const _frame = function(state, subjects, frame, parent, property) {
    // validate the frame
  ...
  const _mergeNodeMaps = function(graphs) {
  ...
  const _createNodeMap = function(input, graphs, graph, issuer, name, list) {
  ...
  const _RDFToObject = function(o, useNativeTypes) {
  ...
  const _isBoolean = function(v) {
    return (typeof v === 'boolean' ||
      Object.prototype.toString.call(v) === '[object Boolean]');
  }

  const _isNumber = function(v) {
    return (typeof v === 'number' ||
      Object.prototype.toString.call(v) === '[object Number]');
  }

  const _isDouble = function(v) {
    return _isNumber(v) && String(v).indexOf('.') !== -1;
  }

  const _objectToRDF = function(item) {
    var object = {};
  ...
  const _listToRDF = function(list, issuer, subject, predicate, triples) {
  ...
  const _graphToRDF = function(graph, issuer, options) {
    var rval = [];
  ...
  const _expandValue = function(activeCtx, activeProperty, value) {
    // nothing to expand
  ...
  const _expandLanguageMap = function(languageMap) {
    var rval = [];
  ...
  const jsonldDOTActiveContextCache = function(size) {
    this.order = [];
    this.cache = {};
    this.size = size || 100;
  };

  jsonldDOTActiveContextCache.prototype.get = function(activeCtx, localCtx) {
  ...
  jsonldDOTActiveContextCache.prototype.set = function(
  ...
  Processor.prototype.createNodeMap = function(input, options) {
    ...
    var issuer = options.namer || options.issuer || new IdentifierIssuer$1('_:b');
    var graphs = {'@default': {}};
  ...
  const _findContextUrls = function(input, urls, replace, base) {
      ...
      for(var key in input) {
        ...
        if(_isArray(ctx)) {
          ...
          for(var i = 0; i < length; ++i) {
            var _ctx = ctx[i];
            if(_isString(_ctx)) {
              _ctx = jsonldDOTprependBase(base, _ctx);
          ...
          // string @context
          ctx = jsonldDOTprependBase(base, ctx);
          // replace w/@context if requested
          if(replace) {
            input[key] = urls[ctx];
          } else if(!(ctx in urls)) {
  ...
  const _retrieveContextUrls = function(input, options, callback) {
  ...
  const jsonldDOTparseLinkHeader = function(header) {
    var rval = {};
  ...
  const jsonldDOTdocumentLoader = (function() {
  ...
  const jsonldDOTprocessContext = function(activeCtx, localCtx) {
    // get arguments
    var options = {};
    var callbackArg = 2;
  ...
  const jsonldDOTcompact = function(input, ctx, options, callback) {
    ...
    var expand = function(input, options, callback) {
      jsonldDOTsetImmediate(function() {
        ...
        jsonldDOTexpand(input, options, callback);
      });
    };
    ...
    expand(input, options, function(err, expanded) {
      ...
      // process context
      var activeCtx = _getInitialContext(options);
      jsonldDOTprocessContext(activeCtx, ctx, options, function(err, activeCtx) {
        if(err) {
          return callback(new JsonLdError(
  ...
  const _removePreserve = function(ctx, input, options) {
  ...
  const jsonldDOTframe = function(input, frame, options, callback) {
    ...
    function doFrame(remoteFrame) {
      ...
      jsonldDOTexpand(input, options, function(err, expanded) {
        ...
        jsonldDOTexpand(frame, opts, function(err, expandedFrame) {
          if(err) {
            return callback(new JsonLdError(
          ...
          opts.link = {};
          jsonldDOTcompact(framed, ctx, opts, function(err, compacted, ctx) {
            if(err) {
              return callback(new JsonLdError(
  ...
  const jsonldDOTlink = function(input, ctx, options, callback) {
  ...
  const _toNQuads = function(dataset) {
    var quads = [];
  ...
  const jsonldDOTnormalize = function(input, options, callback) {
    ...
    if('inputFormat' in options) {
      ...
      jsonldDOTtoRDF(input, opts, function(err, dataset) {
        if(err) {
          return callback(new JsonLdError(
  ...
  const _labelBlankNodes = function(issuer, element) {
    if(_isArray(element)) {
      for(var i = 0; i < element.length; ++i) {
  ...
  const jsonldDOTrelabelBlankNodes = function(input, options) {
    options = options || {};
    var issuer = options.namer || options.issuer || new IdentifierIssuer$1('_:b');
  ...
  const jsonldDOTpromisify = function(op) {
    if(!jsonld.Promise) {
      try {
  ...
  const jsonldDOTpromises = function(options) {
    options = options || {};
  ...
  const JsonLdProcessor$1 = function() {}

  JsonLdProcessor$1.prototype = jsonldDOTpromises({version: 'json-ld-1.0'});


  JsonLdProcessor$1.prototype.toString = function() {
    if(this instanceof JsonLdProcessor$1) {
      return '[object JsonLdProcessor]';
    }
    return '[object JsonLdProcessorPrototype]';
  };

  const jsonldDOTJsonLdProcessor = JsonLdProcessor$1;

  const jsonldDOTRequestQueue = function() {
    this._requests = {};
  };
  ...
  const _parseRdfaApiData = function(data) {
    var dataset = {};
    dataset['@default'] = [];
  ...
  const jsonldDOTregisterRDFParser = function(contentType, parser) {
  ...
  const jsonldDOTunregisterRDFParser = function(contentType) {
    delete _rdfParsers[contentType];
  };

  const jsonldDOTIdentifierIssuer = IdentifierIssuer$1;

  const jsonldDOTUniqueNamer = IdentifierIssuer$1;

  const compact = jsonldDOTcompact;
  const expand = jsonldDOTexpand;
  const flatten = jsonldDOTflatten;
  const frame = jsonldDOTframe;
  const link = jsonldDOTlink;
  const normalize = jsonldDOTnormalize;
  const fromRDF = jsonldDOTfromRDF;
  const toRDF = jsonldDOTtoRDF;
  const createNodeMap = jsonldDOTcreateNodeMap;
  const merge = jsonldDOTmerge;
  const relabelBlankNodes = jsonldDOTrelabelBlankNodes;
  const prependBase = jsonldDOTprependBase;
  const documentLoader = jsonldDOTdocumentLoader;
  ...
  const cache = jsonldDOTcache;
  const processContext = jsonldDOTprocessContext;
  const hasProperty = jsonldDOThasProperty;
  const hasValue = jsonldDOThasValue;
  const addValue = jsonldDOTaddValue;
  const getValues = jsonldDOTgetValues;
  const removeProperty = jsonldDOTremoveProperty;
  const removeValue = jsonldDOTremoveValue;
  const compareValues = jsonldDOTcompareValues;
  const getContextValue = jsonldDOTgetContextValue;
  const registerRDFParser = jsonldDOTregisterRDFParser;
  const unregisterRDFParser = jsonldDOTunregisterRDFParser;
  const IdentifierIssuer = jsonldDOTIdentifierIssuer;
  const UniqueNamer = jsonldDOTUniqueNamer;
  const url = jsonldDOTurl;

var jsonldModule = Object.freeze({
  	compact: compact,
  	expand: expand,
  	flatten: flatten,
  	frame: frame,
  	...
  	merge: merge,
  	relabelBlankNodes: relabelBlankNodes,
  	...
  	registerRDFParser: registerRDFParser,
  	unregisterRDFParser: unregisterRDFParser,
